...
{
    public class ParticleEmitterPixel : RenderableComponent, IUpdatable
    {
        ...
        /// </summary>
        public event Action<ParticleEmitterPixel> OnAllParticlesExpired;

        /// <summary>
        /// event that's going to be called when emission is stopped due to reaching duration specified in ParticleEmitterConfig
        /// </summary>
        public event Action<ParticleEmitterPixel> OnEmissionDurationReached;
        ...
        List<ParticlePixel> _particles;
        bool _playOnAwake;
        [Inspectable] ParticleEmitterConfig _emitterConfig;


        public ParticleEmitterPixel() : this(new ParticleEmitterConfig())
        {
        }

        public ParticleEmitterPixel(ParticleEmitterConfig emitterConfig, bool playOnAwake = true)
        {
            _emitterConfig = emitterConfig;
            _playOnAwake = playOnAwake;
            _particles = new List<ParticlePixel>((int)_emitterConfig.MaxParticles);
            Pool<ParticlePixel>.WarmCache((int)_emitterConfig.MaxParticles);
        ...
        public virtual void Update()
        {
            ...
            for (var i = _particles.Count - 1; i >= 0; i--)
            {
                ...
                {
                    Pool<ParticlePixel>.Free(currentParticle);
                ...
                else
                {
                    ...
                    maxParticleSize = 1;//Math.Max(maxParticleSize, currentParticle.particleSize);
                }
            }
            ...
            for (var i = 0; i < _particles.Count; i++)
            {
                ...
                if (_emitterConfig.Sprite == null)
                    batcher.Draw(Graphics.Instance.PixelTexture, pos + currentParticle.position, currentParticle.color,
                        currentParticle.rotation, Vector2.One, currentParticle.particleSize * 1f, SpriteEffects.None,
                        LayerDepth);
                else
        ...
        {
            for (var i = 0; i < _particles.Count; i++)
                Pool<ParticlePixel>.Free(_particles[i]);
            _particles.Clear();
        }
        ...
        public void EmitFromTexture(Texture2D texture)
        {
            var rootPosition = Entity.Transform.Position + _localOffset;
            ...
            for (int i = 0; i < colors.Length; i+=2)
            {
				...
				Color c = colors[i];
                //add some random blood
                if(Random.Chance(0.3f) && c.A != 0)
                 c = Color.FromNonPremultiplied(195, 0, 0, 255);
				Color premultColor = c;//Color.FromNonPremultiplied(c.R, c.G, c.B, c.A);
                AddColoredParticle(rootPosition + new Vector2(x+1, y+1), premultColor);
            }
        }
        ...
        // say C is at i == 13
        // and width == 5
        // to get positions x == 3 and y == 2 :
        // int x = i % width == 13 % 5 == 3 
        // int y = i / width == 13 / 5 == 2
        ...
        void AddParticle(Vector2 position)
        {
            ...
            var particle = Pool<ParticlePixel>.Obtain();
            particle.Initialize(_emitterConfig, position);
            _particles.Add(particle);
        }

        void AddColoredParticle(Vector2 position, Color color)
        {
            // take the next particle out of the particle pool we have created and initialize it
            var particle = Pool<ParticlePixel>.Obtain();
			_emitterConfig.StartColor = color;
			_emitterConfig.FinishColor = color;
            particle.Initialize(_emitterConfig, position);
            _particles.Add(particle);
        }
    }
